...
export const createUserWithEmail = (data) =>{
    return (dispatch) =>{
        ...
        createUserWithEmailAndPassword(auth, data.email, data.passwordConf).then(async({user})=>{
            const newUser = {
    ...
    return async (dispatch)=>{
        ...
        try{
           ...
           dispatch(toggleLoading())
            ...
            console.log(error)
            dispatch(updatePhoto('https://res.cloudinary.com/dinmmqdca/image/upload/v1681566217/rf8wp1jr4fxdtiq94rsb.png', {status: true, message: error.message}))
            dispatch(toggleLoading())
        }
    }
}
        ...
        try {
            ...
            uid: user.uid,
              ...
              if(!userDoc){
                  ...
                  dispatch(loginUser({...newUser, id: userBase.id}, {status: undefined, message: ''}))
              } else{
                newUser = userDoc
